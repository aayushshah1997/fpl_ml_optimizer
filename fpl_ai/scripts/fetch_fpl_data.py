...
This script can be run weekly to fetch new gameweek data from the FPL API
and save it in vaastav format for seamless integration with our system.
"""
...
def fetch_and_save_gw(gw: int):
    """Fetch data from FPL API for a specific gameweek and save in vaastav format."""
    logger.info(f"Starting GW{gw} data fetch from FPL API...")
    
    # Fetch data
    gw_data = fetch_fpl_data_for_gw(gw)
    
    if gw_data.empty:
        logger.error(f"Failed to fetch GW{gw} data from FPL API")
        return False
    
    # Save in vaastav format
    success = save_vaastav_format(gw_data, gw)
    
    if success:
        logger.info(f"Successfully saved GW{gw} data: {len(gw_data)} players")
        
        # Show sample statistics
        logger.info(f"GW{gw} stats: avg points={gw_data['total_points'].mean():.1f}, "
                   f"max={gw_data['total_points'].max()}, "
                   f"min={gw_data['total_points'].min()}")
        ...
        logger.error(f"Failed to save GW{gw} data")
        return False


def main():
    """Main function to handle command line arguments."""
    parser = argparse.ArgumentParser(description='Fetch FPL data for a specific gameweek')
    parser.add_argument('gameweek', type=int, help='Gameweek number to fetch')
    parser.add_argument('--auto', action='store_true', help='Automatically detect current gameweek')
    
    args = parser.parse_args()
    
    if args.auto:
        # Try to detect current gameweek from FPL API
        try:
            client = FPLAPIClient()
            bootstrap = client.get_bootstrap_data()
            if bootstrap and 'events' in bootstrap:
                ...
                if current_gw:
                    ...
                    success = fetch_and_save_gw(current_gw)
                else:
                    logger.error("Could not detect current gameweek")
                    success = False
            else:
                logger.error("Could not get events data from FPL API")
                success = False
        except Exception as e:
            logger.error(f"Error auto-detecting gameweek: {e}")
            success = False
    else:
        success = fetch_and_save_gw(args.gameweek)
    
    if success:
        print(f"✅ GW{args.gameweek if not args.auto else 'auto'} data successfully fetched and saved in vaastav format!")
    else:
        print(f"❌ Failed to fetch and save GW{args.gameweek if not args.auto else 'auto'} data")
